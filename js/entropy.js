...
window.Entropy = new (function () {
			...
			{},
			{
				get: (_, char) => {
					if (typeof char === "string" && char.length === 1) {
						return char
							.codePointAt(0) // Get the Unicode code point of the character
							.toString(2) // Convert to binary string
							.padStart(8, "0"); // Pad with leading zeros to ensure 8 bits
					}
					return undefined;
				},
			},
	...
	this.fromString = (rawEntropyStr, baseStr) => {
		...
		var entropyBin = base.events
			.map((e) => eventBits[base.str][e.toLowerCase()])
			.join("");
	...
	function getBase(str, baseStr) {
		...
		if (
			...
			var ints = binaryMatches.map((i) => parseInt(i, 2));
			return {
				ints: ints,
		...
		if (
			...
			var ints = diceMatches.map((i) => parseInt(i));
			return {
				ints: ints,
				events: diceMatches,
		...
		if (
			...
			var ints = base6Matches.map((i) => parseInt(i));
			return {
				ints: ints,
				events: base6Matches,
		...
		if (
			...
			var ints = base10Matches.map((i) => parseInt(i));
			return {
				ints: ints,
